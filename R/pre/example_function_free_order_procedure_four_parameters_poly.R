#' four_parameters_ploy
#'
#' This function conducts the free order procedure for any PDF using four
#' parameters...
#'
#' This function is used within \code{\link{poly_wrapper}}..
#'
...
#' four_parameters_ploy(model = rawscore ~ age,
#' data = Data_norm,
#' criterion = "BIC",
#' family = "BCPE")
#'
four_parameters_ploy <- function(model, ..., data, family, criterion) {
  ...
  if(best_model == "initial") {
    ...
    model_result <- eval(parse(text = paste(best_model_split[1],
                                            "model",
                                            best_model_split[2],
                                            "output",
                                            sep = ".", collapse = "")))
